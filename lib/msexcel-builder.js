...
Sheet = (function() {
  ...
  Sheet.prototype.setDate = function(col, row, date) {
    if (!(date instanceof Date)) date = new Date()
    this.data[row][col].d = true
    return this.data[row][col].v = date.toISOString()
  };

  Sheet.prototype.setFormula = function(col, row, formula, value) {
    if (formula != null && formula !== '') {
      if (value !== null) {
        this.data[row][col].f = true
  ...
  Sheet.prototype.dateFmt = function(col, row) {
    var id, inx, style;
    inx = '_' + col + '_' + row;
    style = {
    ...
    for (i = 1, _ref2 = this.rows; 1 <= _ref2 ? i <= _ref2 : i >= _ref2; 1 <= _ref2 ? i++ : i--) {
      ...
      for (j = 1, _ref3 = this.cols; 1 <= _ref3 ? j <= _ref3 : j >= _ref3; 1 <= _ref3 ? j++ : j--) {
        ...
        if ((ix.v !== 0) || (sid !== 1)) {
          ...
          if (ix.v !== 0 && ix.v && !ix.n && !ix.f && !ix.d) {
            c.att('t', 's');
            c.ele('v', '' + (ix.v - 1));
          } else if (ix.v !== 0 && ix.v && ix.n) {
            c.att('t', 'n')
            c.ele('v', String(ix.v))
          } else if (ix.v !== 0 && ix.v && ix.f) {
            c.att('t', 'str')
            c.ele('f', ix.formula)
            c.ele('v', ix.v)
          } else if (ix.v !== 0 && ix.v && ix.d) {
            var sid = this.dateFmt(j, i)
            c.att('s', ''+ (sid-1))
            c.att('t', 'd')
            c.ele('v', ix.v)
          }
...
Style = (function() {
  ...
  Style.prototype.style2id = function(style) {
    ...
    if (style.num_fmt_id) k += '_'+style.num_fmt_id
    id = this.cache[k];
    if (id) {
      return id;
    } else {
  ...
  Style.prototype.toxml = function() {
    ...
    for (_l = 0, _len4 = _ref4.length; _l < _len4; _l++) {
      ...
      if (o.num_fmt) e.att('applyNumberFormat', '1')
